...
local function on_created(event)
	...
	if entity.name ~= "storage-unit" then return end
	local position = entity.position
	local surface = entity.surface
	local force = entity.force

	local combinator = surface.create_entity{
		name = "storage-unit-combinator",
...
local function on_destroyed(event)
	...
	if buffer and item and count ~= 0 then
		buffer.clear()
		buffer.insert("storage-unit-with-tags")
		local stack = buffer.find_item_stack("storage-unit-with-tags")
		stack.tags = {name = item, count = count}
		stack.custom_description = {
			"item-description.storage-unit-with-tags",
			compactify(count),
			item
		}
	end
