...
function update_unit(unit_data, unit_number, force)
  ...
  game.print(type(unit_data.item))
...
script.on_nth_tick(60 / updates_per_tick, function(event)
	local smooth_ups = event.tick % updates_per_tick
...
script.on_event(defines.events.on_player_selected_area, function(event)
  ...
  for e, entity in pairs(event.entities) do
    ...
    -- if container and not already efficient storage
    if entity.type == "container" and entity.name:sub(0,9) ~= "efficient" and prototypes.entity["efficient-" .. entity.name .. "-combinator"] ~= nil then
      ...
      if #items <= 1 then
        ...
        local unit_data = {
          ...
          count = count,
          combinator = combinator,
          stack_size = item and prototypes.item[item] and prototypes.item[item].stack_size or nil,
          mean = item and prototypes.item[item] and prototypes.item[item].stack_size * #container.get_inventory(defines.inventory.chest) / 2 or nil,
          inventory = container.get_inventory(defines.inventory.chest),
          lag_id = math.random(0, updates_per_tick - 1)
        }
        storage.units[combinator.unit_number] = unit_data
